...
pub fn read_string<P: AsRef<Path>>(filename: P) -> Result<String> {
    ...
    let mut handle = File::open(&filename)
        .unwrap_or_else(|_| File::create(&filename).expect("Unable to create file"));
...
impl Transform for String {
    fn transform(mut self, transform: Transforms) -> Self {
        match transform {
            ...
            Transforms::NoWhitespaces => self.replace(" ", "-"),
        }
    }
}
